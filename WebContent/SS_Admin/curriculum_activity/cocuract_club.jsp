		...
		<tr>
			<td style="width: 30%; padding-left: 5px;" valign="top">
				...
				<div id="saveEditForm" class="formboarder" align="center">					
						...
						Department Under
						<table cellpadding="0" cellspacing="0">
							<s:select id="depid" list="depList" listKey="dep_id" listValue="dep_name" headerValue="---" headerKey="0" style="width: 205px; height: 30px;"/>
						</table>
						
						<table cellpadding="0" cellspacing="0" style="padding-top: 5px;">
							<s:submit onclick="saveCocurActClub()" value="Save" style="width: 205px; height: 30px;"/>
						</table>
					...
					<table id="dptList" width="100%" cellpadding="0" cellspacing="1">
						<thead style="background-color: #f5f5f5;">
							<tr style="height: 35px;">
								...
								<th width="35%">About the club</th>
								<th width="15%">Department</th>
								<th width="10%">Status</th>
								<th width="10%">&nbsp;</th>
							</tr>
						</thead>
						<s:iterator status="stat" value="clList">
							<tr style="height: 40px;">
								<td align="center">
									<s:property value="%{#stat.index + 1}"/>
									<s:set var="depid" value="clList[#stat.index].clb_id"/>
								</td>
								<td>
									<s:property value="clList[#stat.index].clb_name"/>
								</td>
								<td>
									<s:property value="clList[#stat.index].clb_desc"/>
								</td>
								<td>
									<s:property value="clList[#stat.index].dep_name"/>
								</td>
								<td>
									<s:set var="clbstatus" value="clList[#stat.index].clb_status"/>
									<s:if test="%{#clbstatus == \"A\"}">Active</s:if>
									<s:else>Inactive</s:else>
								</td>
								<td style="text-align: center;">
									<div style="color: #3d6e9f; cursor: pointer;" onclick="clubEditForm('${depid}')">Edit</div>
								</td>
							</tr>
						</s:iterator>
					</table>
