...
public class DatabaseBackup extends TimerTask {
	...
	public static boolean backupDataWithDatabase(String dumpExePath, String host, String port, String user, String password, String database, String backupPath){
		...
		try{
			...
			String filename = "backup(with_DB)-" + database + "-" + host + "-(" + dateFormat.format(date) + ").sql";
			
			Runtime runtime = Runtime.getRuntime();
			
			p = runtime.exec(new String[]{dumpExePath, "-uroot", "-pP@55yamget", database, "-r\"" + backupPath + "" + filename + "\"" });
	...
	public void run() {
		...
		backupDataWithDatabase(SysConstant.DUMP_EXE_PATH, SysConstant.HOST, SysConstant.PORT, SysConstant.DATABASE_USERNAME, SysConstant.DATABASE_PASSWORD, SysConstant.DATABASE_NAME, SysConstant.DATABASE_BACKUP_FOLDER);
	}
